...
//    }
    ...
    //label - для иконки врага
    //label2 - для имени врага
    //text - дамаг врага
    //label 3 - здоровье/макс. здоровье
    //иконка это конкретное поле

    public Player ChooseEnemy() {
        int i = (int) (Math.random() * 4);
        switch (i) {
            case 0 -> {
                enemy = new BarakaFactory().createEnemy();
            }
            case 1 -> {
                enemy = (Player) new SubZeroFabric().createEnemy();
            }
            case 2 -> {
                enemy = (Player) new LiuKangFabric().createEnemy();
            }
            case 3 -> {
                enemy = (Player) new SonyaBladeFabric().createEnemy();
            }
        }
        return enemy;
    }
    ...
    public Player ChooseBoss(JLabel label, JLabel label2, JLabel text, JLabel label3, int i) {
        ...
        switch (i) {
            case 2:
                enemy = (Player) new ShaoKahnFabric().createEnemy();
                break;
            case 4:
                //ShaoKahn посильнее
                enemy = (Player) new ShaoKahnFabric().createEnemy();
